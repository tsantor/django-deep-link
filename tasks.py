...
def coverage(c):
    ...
    """
    c.run("coverage run --source django-deep-link runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/django-deep-link.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ django_deep_link")
...
def lint(c):
    ...
    """
    c.run("flake8 django-deep-link tests")
...
def release(c, bumpsize=''):
    ...
    import django_deep_link
    c.run("python setup.py sdist bdist_wheel")
    c.run("twine upload dist/*")

    c.run('git tag -a {version} -m "New version: {version}"'.format(version=django_deep_link.__version__))
    c.run("git push --tags")
